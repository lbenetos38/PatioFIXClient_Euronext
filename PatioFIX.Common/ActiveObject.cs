...
namespace PatioFIX.Common
{
    ...
    public class ActiveObject
    {
        ...
        public ActiveObject(string threadName, int timerInterval = 0)
        {
            ...
            //we create an inactive timer...
            m_timerInterval = timerInterval;
            m_timer = new Timer((state) =>
            {
                //This method executes on a ThreadPool thread supplied by the system
                this.NumberOfHeartBeats++;
                m_beatEvent.Set();
            }, null, dueTime: Timeout.Infinite, period: Timeout.Infinite);
            ...
            if (m_issuedAuthoritativeStop == false)
            {
                lock (m_timerLock)
                {
                    if (m_issuedAuthoritativeStop == false)
                    {
                        ...
                        m_timer.Change(dueTime: (startImmediately ? 0 : GetTimerInterval()), period: Timeout.Infinite);
                    }
                }
            }
        }
        ...
        protected void StopTimer()
        {
            lock (m_timerLock)
            {
                ...
                m_timer.Change(dueTime: Timeout.Infinite, period: Timeout.Infinite);
            }
        }
        /// <summary>
        /// 
