...
namespace PatioFIX.Common.BLL.Evaluators
{
    ...
    /// 
    /// </summary>
    class ExchangeNotesEvaluator : BaseEvaluator
    {
        ...
        public ExchangeNotesEvaluator(IOdlDataLayer datalayer) : base(datalayer)
        {
            theLogger = new Logger("ExchangeNotesEvaluator");
        }
        ...
        public override EvaluationResult Evaluate(FIXInMessage fixInMessage, IODLMessage odlMessage)
        {
            ...
            if (fixInMessage.ODLMessageType != ODLMessageTypeEnum.Exchange_Notes) throw new PtBusinessException($"InvalidMessageType. Need Exchange_Notes but received {fixInMessage.ODLMessageType}");
            #endregion

            try
            {
                //Κανουμε cast το comObject στον σωστο συγκεκριμένο τύπο
                var exchangeNotes = (ExchangeNotesMessage)odlMessage;


                OdlDal.InsertExchangeNotes(fixInMessage, exchangeNotes);
            }
