...
namespace PatioFIX.Common.FixSupport
{
    public class FIXMessageWriter
    {
        ...
        public FIXMessageWriter Set(int tag, char value)
        {
            _bodyEnd += _body.WriteInt(_bodyEnd, tag);
            _body[_bodyEnd++] = EQL;

            _bodyEnd += _body.WriteChar(_bodyEnd, value);
            _body[_bodyEnd++] = SOH;
        ...
        public FIXMessageWriter Set(int tag, long value)
        {
            ...
            _bodyEnd += _body.WriteLong(_bodyEnd, value, negative: value >= 0 ? false : true);
            _body[_bodyEnd++] = SOH;
        ...
        public FIXMessageWriter Set(int tag, decimal value, int decimals = 6)
        {
            _bodyEnd += _body.WriteInt(_bodyEnd, tag);
            _body[_bodyEnd++] = EQL;

            _bodyEnd += _body.WriteDecimal(_bodyEnd, value, decimals);
            _body[_bodyEnd++] = SOH;
        ...
        {
            return CharEncoding.DefaultEncoding.GetString(Buffer, 0, Length);
        }
