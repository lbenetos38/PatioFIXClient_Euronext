...
namespace PatioFIX.Common.FixSupport.Transport
{
    ...
    /// Simple Socket
    /// </summary>
    public class TCPConnection3 : IDisposable
    {
        ...
        void _ReceiveLoop()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    int numOfBytes = m_socket.Receive(_rcvBuffer, _rcvBuffer.Length, SocketFlags.None);
                    if (numOfBytes > 0)
                    {
            ...
            if (m_socket != null)
            {
                lock (_sync)
                {
                    if (m_socket != null)
                    {
                        ...
                        {
                            /*
                             * Μπορει το socket να εχει γίνει ηδη shautdown
