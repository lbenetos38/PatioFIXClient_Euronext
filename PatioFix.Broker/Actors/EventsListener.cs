...
namespace PatioFix.Broker
{
    ...
    internal sealed class EventsListener : ActiveObject
    {
        ...
        void OnNewMessage(FIXMessage inbound, ODLMesssageSource source)
        {
            ...
            if (Evaluator.DoWeIgnoreMessageType(ODLMesssageSource.Broker, messageType) == true)
            {
                ...
                /*
                 * Aλλαζουμε το MessageType σε ignored και το βαζουμε στο QUEUE μαq
                 * Αυτο το κανουμε για να αποθηκευσουμε στην βαση μας το MsgSeqNum και το MessageID
                 */
                var message = new FIXInMessage(inbound, source, ODLMessageTypeEnum.Ignored_Message);
        ...
        protected override void OnQueueEvent()
        {
            ...
            try
            {
                ...
                while (_queue.TryPeek(out p_message))
                {
                    ...
                    /*
                     * επειδη μεσα στο _queue εχουμε και FIXmessages που κανονικα θα τα αγνοουσαμε,
                     * (αλλα τα ψευδοεπεξεργαζομαστε για να αποθηκευσουμε στην βαση το το MsgSeqNum και το MessageID)
                     * σε αυτο το σημειο δεν τα εμφανιζουμε στο log...
                     */
                    if (p_message.ODLMessageType != ODLMessageTypeEnum.Ignored_Message)
                    {
                    ...
                    try
                    {
                        ...
                        else if (evaluationResult == EvaluationResult.RetryableSqlException)
                        {
							 ...
							 * Μπορούμε να δοκιμάσουμε να ξανακάνουμε evaluation το FIXInMessage με την ελπιδα ότι οποιο προβλημα
							 * με τον SQL Server θα αποκατασταθεί συντόμως (μπορει να ηταν deadlock ή ενα timeout)
							 * Για να κανουμε evaluation ξανα το FIXInMessage απλα δεν το κανουμε Dequeue (παραμενει στο _queue, και θα 
